...
TEST_CERT_DIR="$CERTS_DIR/selfsigned/live/example.com_8443"
mkdir -p "$TEST_CERT_DIR"
printf 'fixture fullchain\n' >"$TEST_CERT_DIR/fullchain.pem"
printf 'fixture privkey\n' >"$TEST_CERT_DIR/privkey.pem"
...
port,example.com,,,,,8443,9443,https,selfsigned/live/example.com_8443,yes,off,,off,auto,,,,,,
path,example.com,,,/app,customset,8443,,,,inherit,off,,,,prefix,100,,none,-,auto
path,example.com,,,/shared,sharedset,8443,,,,inherit,off,,,,prefix,100,,none,-,auto
path,other.com,,,/shared,sharedset,8443,,,,inherit,off,,,,prefix,100,,none,-,auto
EOF_PORTS
...
_sr_write_db_line 0 "Example.COM" "single" "429" "1" "uri" "/allow" "contains" "ok" >>"$SECURITY_RULES_DB"
...
key,docker_options
backend:example.com,--restart=always
backend:other.com,--cpus=1
EOF_DOCKER_OPTS
...
if grep -q '^8443' "$PORT_TLS_PROTOCOLS_FILE" 2>/dev/null; then
  echo "[Error] TLS protocol override for port 8443 not removed" >&2
  exit 1
fi

if grep -q '^8443' "$PORT_TLS_CIPHERS_FILE" 2>/dev/null; then
  echo "[Error] TLS cipher override for port 8443 not removed" >&2
  exit 1
fi

if grep -q 'backend:example.com' "$BACKEND_DOCKER_OPTS_FILE" 2>/dev/null; then
  echo "[Error] Backend Docker opts not cleared" >&2
  exit 1
fi
...
if grep -q 'example.com' "$BACKENDS_CONF" 2>/dev/null; then
  echo "[Error] Removed backend still present in generated config" >&2
  exit 1
fi

if grep -q 'customset' "$BACKENDS_CONF" 2>/dev/null; then
  echo "[Error] Stale path header include detected" >&2
  exit 1
fi

printf 'Remove-backend regression checks passed.\n'
