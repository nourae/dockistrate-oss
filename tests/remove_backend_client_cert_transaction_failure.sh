...
TMP_ROOT="$(_realpath_portable "$(mktemp -d "${TMPDIR:-/tmp}/dockistrate_remove_client_fail.XXXXXX")")"
...
function _revoke_backend_client_cert() {
  echo "[Error] Forced client removal failure" >&2
  return 1
}
...
set +E
output="$(
  (remove_backend_client_cert example.com client1) 2>&1
)"
status=$?
set -E
set -e
...
if [ "$status" -eq 0 ]; then
  echo "[Error] remove_backend_client_cert succeeded unexpectedly." >&2
  exit 1
fi
...
if [ -e "$UPDATE_MARKER" ] || [ -L "$UPDATE_MARKER" ]; then
  echo "[Error] remove_backend_client_cert continued into update_nginx_config after helper failure." >&2
  exit 1
fi

if ! grep -Fq "Rolled back" <<<"$output"; then
  echo "[Error] Expected rollback message missing for remove-backend-client-cert failure." >&2
  echo "$output" >&2
  exit 1
fi

if grep -Fq "Removed client certificate client1 for example.com" <<<"$output"; then
  echo "[Error] remove_backend_client_cert reported success after helper failure." >&2
  echo "$output" >&2
  exit 1
fi

echo "remove-backend-client-cert aborts before update/success paths when revoke helper fails."
