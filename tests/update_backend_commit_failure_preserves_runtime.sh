...
TMP_ROOT="$(mktemp -d "${TMPDIR:-/tmp}/dockistrate_update_backend_commit_fail.XXXXXX")"
...
docker_log_file="$TMP_ROOT/docker_update_backend_commit_failure.log"
...
function docker() {
  ...
  stop)
    ...
    if [ "$target_stop" = "$test_backend_name" ] && [ "$(read_container_meta "$active_meta_file" exists || true)" = "true" ]; then
      write_container_meta "$active_meta_file" "true" "$test_backend_name" "exited" "$(read_container_meta "$active_meta_file" id || true)"
      return 0
    fi
    return 1
    ;;
  start)
    ...
    if [ -n "$backup_name" ] && [ "$target_start" = "$backup_name" ] && [ "$backup_exists" = "true" ]; then
      write_container_meta "$backup_meta_file" "true" "$backup_name" "running" "$backup_id"
      return 0
    fi
    return 1
    ;;
...
set +e
output="$(update_backend "$domain" --image hashicorp/http-echo 2>&1)"
status=$?
set -e

if [ "$status" -eq 0 ]; then
  echo "[Error] update_backend succeeded unexpectedly when post-change backup finalization was forced to fail." >&2
  exit 1
fi

if ! printf '%s' "$output" | grep -Fq "failed. Rolled back."; then
  echo "[Error] Expected update_backend rollback output when commit finalization fails." >&2
  exit 1
fi
...
if ! grep -Fq "subcommand=start start $cname" "$docker_log_file"; then
  echo "[Error] update_backend rollback did not restart the restored original container." >&2
  exit 1
fi

if ! grep -Fq "subcommand=rename rename $rollback_name $cname" "$docker_log_file"; then
  echo "[Error] update_backend rollback did not restore the original container name." >&2
  exit 1
fi

if ! grep -Fq "subcommand=rm rm -f -v $cname" "$docker_log_file"; then
  echo "[Error] update_backend rollback did not remove the failed replacement container." >&2
  exit 1
fi
...
if grep -Fq "subcommand=rm rm -f $rollback_name" "$docker_log_file"; then
  echo "[Error] update_backend removed the rollback container before commit finalization completed." >&2
  exit 1
fi

if ! grep -Fq "backend,example.com,10.0.0.5:8000,dockistrate-net" "$BACKEND_PORTS_FILE"; then
  echo "[Error] Backend config was not restored after end_transaction_success failure." >&2
  exit 1
fi

if ! cmp -s "$BACKEND_DOCKER_OPTS_FILE" "$BACKEND_DOCKER_OPTS_FILE.orig"; then
  echo "[Error] Backend docker opts were not restored after end_transaction_success failure." >&2
  exit 1
fi

echo "update_backend preserves runtime rollback state until transaction commit succeeds."
