...
BACKUP_ARCHIVE_FILE="${BACKUP_DIR}/sample.tar.gz"
PCAP_FILE="${CAPTURE_DIR}/sample.pcap"
...
if output="$(
  cd "$ROOT_DIR" && \
    PATH="${ROOT_DIR}/tests/mocks:$PATH" SKIP_DOCKER_CHECKS=true ./dockistrate.sh status 2>&1
)"; then
  status=0
else
  status=$?
...
if [ "$status" -ne 0 ]; then
  printf 'Expected status command to succeed, got exit %s\n%s\n' "$status" "$output" >&2
  exit 1
fi
...
echo "Runtime state default permission checks passed."
