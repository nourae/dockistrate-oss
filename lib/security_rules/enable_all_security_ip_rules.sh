...
function _enable_all_security_ip_rules_rewrite_row() {
  CSV_FIELDS[0]="1"
  return 0
}

function enable_all_security_ip_rules() {
  ...
  if ! _config_begin_transaction_if_needed started_txn "enable_all_security_ip_rules"; then
    exit 1
  fi
  csv_rewrite_rows "$SECURITY_IP_RULES_DB" "$STATE_SECURITY_IP_RULES_HEADER" "$STATE_SECURITY_IP_RULES_COLS" _enable_all_security_ip_rules_rewrite_row || return 1
  echo "[Info] Enabled all security IP rules."
  create_backup "" "EnableAllSecIP"
