...
function enable_security_rule() {
  local id="${1:-}"
  [[ "$id" =~ ^[0-9]+$ ]] || {
    echo "[Usage] enable-security-rule <id>"
    exit 1
  }
  ...
  if ! _config_begin_transaction_if_needed started_txn "enable_security_rule_${id}"; then
    exit 1
  fi
  CSV_FIELDS[0]="1"
  _sr_replace_line "$id" "$(csv_join_row "${CSV_FIELDS[@]}")"
  echo "[Info] Enabled rule $id"
  create_backup "" "EnableSecRule_${id}"
