...
function remove_all_security_rules() {
  ...
  if ! _config_begin_transaction_if_needed started_txn "remove_all_security_rules"; then
    exit 1
  fi
  printf '%s\n' "$STATE_SECURITY_RULES_HEADER" >"$SECURITY_RULES_DB"
  echo "[Info] Removed all security rules."
  create_backup "" "RemoveAllSecRules"
