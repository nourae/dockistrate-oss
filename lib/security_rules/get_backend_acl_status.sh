...
function get_backend_acl_status() {
  local domain="${1:-}"
  [ -n "$domain" ] && domain="$(normalize_domain "$domain")"
  local val="$ACL_STATUS"
  local has_explicit_status="no"
  if [ -n "$domain" ] && [ -f "$BACKEND_ACL_STATUS_FILE" ]; then
    local custom=""
    custom="$(state_csv_get_two_col_value "$BACKEND_ACL_STATUS_FILE" "$STATE_BACKEND_ACL_STATUSES_HEADER" "$domain" "" 2>/dev/null || true)"
    if [ -n "$custom" ]; then
      val="$custom"
      has_explicit_status="yes"
    fi
  fi
  # For dedicated hosts, fall back to target domain's ACL status if:
  ...
  if [ "$has_explicit_status" = "no" ]; then
    ...
    if [ -n "$target_domain" ]; then
      ...
      if [ "$should_inherit" = "yes" ] && [ -f "$BACKEND_ACL_STATUS_FILE" ]; then
        local target_custom=""
        target_custom="$(state_csv_get_two_col_value "$BACKEND_ACL_STATUS_FILE" "$STATE_BACKEND_ACL_STATUSES_HEADER" "$target_domain" "" 2>/dev/null || true)"
