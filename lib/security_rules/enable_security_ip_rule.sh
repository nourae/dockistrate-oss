...
function enable_security_ip_rule() {
  local id="${1:-}"
  if ! [[ "$id" =~ ^[0-9]+$ ]]; then
    echo "[Usage] enable-security-ip <id>"
    exit 1
  fi
  [ -f "$SECURITY_IP_RULES_DB" ] || {
    echo "[Error] No security IP rules to enable." >&2
    exit 1
  }
  local line
  ...
  if ! _config_begin_transaction_if_needed started_txn "enable_security_ip_${id}"; then
    exit 1
  fi
  CSV_FIELDS[0]="1"
  _sr_ip_replace_line "$id" "$(csv_join_row "${CSV_FIELDS[@]}")"
  echo "[Info] Enabled security IP rule $id"
  create_backup "" "EnableSecIP_${id}"
