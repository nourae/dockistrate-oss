...
function revoke_backend_client_cert() {
  local domain="${1:-}" client="${2:-}"
  if [ -z "$domain" ] || [ -z "$client" ]; then
    echo "[Usage] revoke-backend-client-cert <domain> <client_name>"
    exit 1
  fi
  ...
  _mtls_begin_transaction_if_needed started_txn "revoke_backend_client_cert_${domain}_${client}" "$mtls_dir" || exit 1
  _revoke_backend_client_cert "$mtls_dir" "$client" false || return 1
  update_nginx_config || return 1
  _mtls_end_transaction_if_started "$started_txn"
  echo "[Info] Revoked client certificate $client for $domain (files retained)"
}
